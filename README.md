# Slides

Vibe coded slides with [reveal.js](https://revealjs.com/) using [revealjs skill](https://github.com/AlahmadiQ8/skills).

## Presentations

<!-- SLIDES-START -->
- [Git Worktrees: Master Parallel Development](https://alahmadiq8.github.io/slides/slides/Git_Worktrees_Guide.html)
- [Five Tribes: The Djinns of Naqala](https://alahmadiq8.github.io/slides/slides/five-tribes.html)
- [Playwright CLI - Token-Efficient Browser Automation for AI Coding Agents](https://alahmadiq8.github.io/slides/slides/playwright-cli.html)
- [Power BI BPA Auto-Fix - GitHub Copilot SDK](https://alahmadiq8.github.io/slides/slides/powerbi-bpa-autofix.html)
<!-- SLIDES-END -->

---

## Adding a New Slide

1. Create a new `.html` file in `slides/` (use an existing deck as a template).
2. Push to `main` - the GitHub Actions workflow will automatically update this README and deploy to GitHub Pages.

## Local Development

Open any slide file directly in your browser:

```sh
open slides/playwright-cli.html
```
